...
- The next seam has shifted from routing to solve quality on that degraded path: fresh stripped-PATH validation now fails at `exploit_l3` with `autofix skipped: missing prerequisites (evidence=0, pie_base=no)`, so the next valuable work is improving recon-only local exploit generation / verify on ret2win-style cases when no debugger evidence exists.
