...
- The remaining gap is still end-to-end validation: add a replayable or scripted real runtime case that proves the stripped-tool path reaches the intended degraded missing-Codex plan (`local_recon_exploit` or similar), not just the planner helper in isolation.
